...
public class OrderManagementController : BaseController
{
    ...
    /// <summary>
    /// Changes the state of an order to Delivered
    /// Only usable by Admins
    /// </summary>
    /// <param name="orderId">The id of the order</param>
    /// <returns>nothing</returns>
    /// <response code="204">If the order state was changed to delivered successfully</response>
    /// <response code="401">If the user is not authorized</response>
    /// <response code="403">If the user is not an admin</response>
    /// <response code="404">If the order does not exist</response>
    /// <response code="500">If an internal server error occurs</response>
    [HttpPut("{orderId}/state/deliver")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(typeof(string), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(string), StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(string), StatusCodes.Status404NotFound)]
