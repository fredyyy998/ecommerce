...
public class CustomWebApplicationFactory<TStartup> : WebApplicationFactory<TStartup> where TStartup : class
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureServices(services =>
        {
            ...
            var kafkaAccountConsumer = services.SingleOrDefault(
                d => d.ImplementationType ==
                     typeof(KafkaAccountConsumer));
            services.Remove(kafkaAccountConsumer);
            
            var kafkaShoppingCartConsumer = services.SingleOrDefault(
                d => d.ImplementationType ==
                     typeof(KafkaShoppingCartConsumer));
            services.Remove(kafkaShoppingCartConsumer);
